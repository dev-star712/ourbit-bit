...
# Market Data
"""get kline"""
client = ourbit_market(ourbit_hosts=hosts)
...
now_price = client.get_price(params)
temp_min = eval(now_price["price"])
temp_max = eval(now_price["price"])

while True:
    price = client.get_price(params)
    price = eval(price["price"])
    if price > temp_max:
        temp_max = price
    ...
    print("max: ", temp_max, "   min: ", temp_min)
